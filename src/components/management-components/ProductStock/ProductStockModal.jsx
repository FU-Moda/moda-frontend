            ...
            <div key={index}>
                ...
                <div className="card-body">
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="flex items-center mb-2">
                      ...
                      <span>{clothTypeLabels[item.product.clothType]}</span>
                    </div>
                    <div className="flex items-center mb-2">
                      <span className="font-semibold mr-2">Giới tính:</span>
                      <span>{genderLabels[item.product.gender]}</span>
                    </div>
                    <div className="flex items-center mb-2">
                      <span className="font-semibold mr-2">Tên cửa hàng:</span>
                      <span>{item.productStock[0].product?.shop?.name}</span>
                    </div>
                    <div className="flex items-center">
                      <span className="font-semibold mr-2">
                        Địa chỉ cửa hàng:
                      </span>
                      <span>{item.productStock[0].product?.shop?.address}</span>
                    </div>
                  </div>
                </div>
              </div>
              <div className="overflow-x-auto">
                <table className="table w-full">
                  ...
                  <tbody>
                    {item.productStock.map((stock, index) => (
                      <tr key={index}>
                        <td>{index + 1}</td>
                        <td>
                          {stock.shoeSize
